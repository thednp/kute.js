...
(function (root, factory) {
  ...
  var defaultOptions = {
    ...
    getInlineStyle = function (el) { // get transform style for element from cssText for .to() method
      /* cmiscm fixed */
      if (!el || !el[style]) return; // if the scroll applies to `window` it returns as it has no styling
    ...
    _requestAnimationFrame = g.requestAnimationFrame || g.webkitRequestAnimationFrame || function (c) {
      return setTimeout(c, 16)
    },
    _cancelAnimationFrame = g.cancelAnimationFrame || g.webkitCancelRequestAnimationFrame || function (c) {
    ...
    color = interpolate.color = function (a, b, v, toHex) { // rgba1, rgba2, progress, convertToHex(true/false)
      var _c = {},
        ...
        rgba = 'rgba(';
      for (c in b) {
        _c[c] = c !== 'a' ? (number(a[c], b[c], v) >> 0 || 0) : (a[c] && b[c]) ? (number(a[c], b[c], v) * 100 >> 0) / 100 : null;
      }
    ...
    update = function (t) {
      ...
      var elapsed = Math.min((t - this._startTime) / this[options][duration], 1),
        progress = this[options][easing](elapsed); // calculate progress
      //console.log(this[element], this[valuesStart],  this[valuesEnd], progress)

      /* cmiscm update */
      for (var tweenProp in this[valuesEnd]) { // render the DOM update
        //console.log(DOM[tweenProp], tweenProp, this[valuesStart][tweenProp], this[valuesEnd][tweenProp])
        //if (DOM[tweenProp]) {
      ...
      //console.log('rrrrr', this[options], elapsed)
      if (elapsed === 1) {
        if (this[options][repeat] > 0) {
          ...
          /* cmiscm fix */
          if (this[options].complete && !this[options].calledComplete) {
            this[options].calledComplete = true;
    ...
    perspective = function () {
      ...
      if (ops.transformOrigin !== undefined && (!('svgTransform' in this[valuesEnd]))) {
    ...
    parseProperty = { // we already start working on core supported properties
        ...
        if (/translate/.test(tweenProp)) {
          if (tweenProp === 'translate3d') {
            ...
            var t1d = trueDimension(inputValue),
              percentOffset = /X/.test(tweenProp) ? this[element][offsetWidth] / 100 : /Y/.test(tweenProp) ? this[element][offsetHeight] / 100 : (this[element][offsetWidth] + this[element][offsetHeight]) / 200;

            return t1d.u === '%' ? (t1d.v * percentOffset) : t1d.v;
          } else if (tweenProp === 'translate') {
            var tv = typeof inputValue === 'string' ? inputValue[split](',') : inputValue,
              t2d = {},
              t2dv,
              t2d0 = trueDimension(tv[0]),
              t2d1 = tv[length] ? trueDimension(tv[1]) : {
                v: 0,
                u: 'px'
              };
      ...
      /* cmiscm add */
      object: function (tweenProp, inputValue) { // object
        if (!(tweenProp in DOM)) {
          DOM[tweenProp] = function (elem, tweenProp, a, b, v) {
            //console.log(a, b, v)
            elem[tweenProp] = number(a, b, v); //(b - a) / 1 * v + a;
          }
        }
        return parseFloat(inputValue);
      }
      ...
      for (var x in obj) {
        if (transformFunctions[indexOf](x) !== -1) { // transform object gets built here
          ...
          if (boxModelProps[indexOf](x) !== -1) {
            ...
            propertiesObject[x] = parseProperty.object.call(this, x, obj[x]);
          }
        }
      }
    },
          ...
          if (/translate/.test(tweenProperty) && tweenProperty !== 'translate') {
            ...
            for (var degIndex = 0; degIndex < 2; degIndex++) {
              for (var axisIndex = 0; axisIndex < 3; axisIndex++) {
                ...
                if (transformFunctions[indexOf](s) !== -1 && (s in this[valuesStart])) {
          ...
          if (sp !== 'perspective') {
            if (typeof this[valuesStart][transformProperty][sp] === 'object') {
              for (var spp in this[valuesStart][transformProperty][sp]) { // 3rd level
                if (typeof this[valuesEnd][transformProperty][sp] === 'undefined') {
  ...
  var Tween = function (targetElement, startObject, endObject, optionsObj) {
    ...
    /* cmiscm add delayedCall */
    delayedCall = function (duration, callback, params, scope) {
      var optionsObj = {
        duration: duration,
      };
      if (params) {
