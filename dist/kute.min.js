...
null)||e.currentStyle,c=F(a),g=b[a]&&!/auto|initial|none|unset/.test(b[a])?b[a]:d[c];if("transform"!==a&&(c in d||c in b))return g?"filter"===c?(b=parseInt(g.split("=")[1].replace(")","")),parseFloat(b/100)):g:v[a]},O=function(e){if("function"===typeof e)return e;if("string"===typeof e)return/easing|linear/.test(e)?f[e]:/bezier/.test(e)?(e=e.replace(/bezier|\s|\(|\)/g,"").split(","),u.Ease.Bezier(1*e[0],1*e[1],1*e[2],1*e[3])):/physics/.test(e)?u.Physics[e]():u.Ease[e]()},aa="ontouchstart"in window||
navigator.msMaxTouchPoints?"touchstart":"mousewheel",ma=F("transformOrigin"),na=F("perspective"),ba=F("perspectiveOrigin"),oa=F("transform"),pa=window.requestAnimationFrame||window.webkitRequestAnimationFrame||function(e){return setTimeout(e,16)},qa=window.cancelAnimationFrame||window.webkitCancelRequestAnimationFrame||function(e){return clearTimeout(e)},A=document.body,K=document.getElementsByTagName("HTML")[0],ca=/webkit/i.test(navigator.userAgent)||"BackCompat"==document.compatMode?A:K,da=8===
(null!=/MSIE ([0-9]{1,}[.0-9]{0,})/.exec(navigator.userAgent)?parseFloat(RegExp.$1):!1),P=["color","backgroundColor"],Q=["scrollTop","scroll"],ea=["opacity"],R=["top","left","width","height"],G="translate3d translateX translateY translateZ rotate translate rotateX rotateY rotateZ skewX skewY scale".split(" "),S=P.concat(Q,ea,R,G),A=S.length,v=v||{},K=0;for(;K<A;K++){var r=S[K];if(-1!==P.indexOf(r))v[r]="rgba(0,0,0,0)";else if(-1!==R.indexOf(r))v[r]=0;else if("translate3d"===r)v[r]=[0,0,0];else if("translate"===
