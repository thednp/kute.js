...
function getLegacyTransform(tweenProperty /* , value */) {
  ...
  return currentStyle[tweenProperty]
    ? currentStyle[tweenProperty]
    : defaultValues[tweenProperty];
}
    ...
    if (arrayFunctions.includes(x)) {
      if (support3DTransform) {
        if (x === "translate3d" || x === "rotate3d") {
          ...
          transformObject.translate3d = pv.length
            ? pv.concat(0)
            : [pv || 0, 0, 0];
        } else if (x === "rotate") {
      ...
      const fnLen = fn === "skew" || (!support3DTransform && fn === "translate")
        ? 2
        : 3;
      let fnArray = [];
      ...
      if (support3DTransform) {
        ...
        transformObject[fnId] = fn === "rotate"
          ? (fnArray[2] || fnArray[1] || fnArray[0])
          : fnArray;
      }
