...
export function prepareShadow(tweenProp, propValue) {
  ...
  if (typeof value === "string") {
    ...
    const colRegEx =
      /(\s?(?:#(?:[\da-f]{3}){1,2}|rgba?\(\d{1,3},\s*\d{1,3},\s*\d{1,3}\))\s?)/gi;
    ...
    value = /inset/.test(value)
      ? value.replace(/(\s+inset|inset+\s)/g, "")
      : value;

    // also getComputedStyle often returns color first "rgb(0, 0, 0) 15px 15px 6px 0px inset"
    value = value.replace(currentColor[0], "").split(" ").concat([
      currentColor[0].replace(/\s/g, ""),
    ], [inset]);
