...
export function prepareAttr(tweenProp, attrObj) { // attr (string),attrObj (object)
  ...
  Object.keys(attrObj).forEach((p) => {
    ...
    const regex = /(%|[a-z]+)$/;
    const currentValue = this.element.getAttribute(
      prop.replace(/_+[a-z]+/, ""),
    );
    ...
    if (!svgColors.includes(prop)) {
      ...
      if (currentValue !== null && regex.test(currentValue)) {
        const unit = trueDimension(currentValue).u ||
          trueDimension(attrObj[p]).u;
        ...
        onStart[ComponentName][prop + suffix] = (tp) => {
          if (
            this.valuesEnd[tweenProp] && this.valuesEnd[tweenProp][tp] &&
            !(tp in attributes)
          ) {
            attributes[tp] = (elem, oneAttr, a, b, v) => {
              ...
              elem.setAttribute(
                _p,
                ((numbers(a.v, b.v, v) * 1000 >> 0) / 1000) + b.u,
              );
            };
          }
        };
        ...
        !regex.test(attrObj[p]) || currentValue === null ||
      ...
      attributesObject[prop] = trueColor(attrObj[p]) ||
        defaultValues.htmlAttributes[p];
    }
  });
