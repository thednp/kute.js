...
export default function linkInterpolation() { // DON'T change
  Object.keys(linkProperty).forEach((component) => {
    ...
    Object.keys(componentLink).forEach((fnObj) => {
      if (
        typeof (componentLink[fnObj]) === "function" && // ATTR, colors, scroll, boxModel, borderRadius
        Object.keys(this.valuesEnd).some((i) =>
          (componentProps && componentProps.includes(i)) ||
          (i === "attr" &&
            Object.keys(this.valuesEnd[i]).some((j) =>
              componentProps && componentProps.includes(j)
            ))
        )
      ) {
            ...
            Object.keys(propObject).forEach((i) => {
              if (typeof (componentLink[i]) === "function") { // transformCSS3
                ...
                Object.keys(componentLink[fnObj]).forEach((j) => {
                  if (
                    componentLink[i] &&
                    typeof (componentLink[i][j]) === "function"
                  ) { // transformMatrix
